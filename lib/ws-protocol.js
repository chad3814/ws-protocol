...
fs.readFile(path.resolve(path.join(__dirname, 'client', 'jdataview.js')), function (err, content) {
    if (err) {
...
fs.readFile(path.resolve(path.join(__dirname, 'client', 'wsprotocol.js')), function (err, content) {
    if (err) {
...
WSProtocol.prototype.broadcast = function (obj) {
...
WSProtocol.prototype.setUpWS = function () {
    var ws_options = {
        server: this.server,
...
WSProtocol.prototype.setUpRequests = function () {
    ...
    this.server.on('request', function (req, res) {
        if (req.url !== client_path) {
            return old_request_listeners.forEach(function (listener) {
        ...
        if (req.headers && req.headers.host) {
            host_port = req.headers.host;
        } else {
            host_port = '';
        }
        var connect_string = '\nWSProtocol.connect_url = "ws://' + host_port + path.join('/', this.options.base_path, '_s') + '";\n';

        var length = 0;
        if (this.options.enable_jdataview) {
            length += Buffer.byteLength(jDataView_client, 'utf8') + 1;
        }
        if (this.options.enable_client) {
            length += Buffer.byteLength(wsprotocol_client, 'utf8');
            length += Buffer.byteLength(connect_string, 'utf8');
        }
